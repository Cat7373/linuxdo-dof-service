...
export async function sendReward(dnfId: number, characId: number | null, reward: DnfReward) {
  ...
  if (reward.items) {
    for (const item of reward.items) {
      await useKnex().raw(`INSERT INTO taiwan_cain_2nd.postal (occ_time, send_charac_name, receive_charac_no, item_id, add_info) VALUES ('${dayjs().format('YYYY-MM-DD HH:mm:ss')}', '${config.dnfMailSender}', ${characId}, ${item.id}, ${item.count})`)
    }
  }
}
